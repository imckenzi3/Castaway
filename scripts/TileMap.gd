	...
	for x in range(width):
		for y in range(height):
			...
			if alt < 0: # Arbitrary sea level value (choosing 0 will mean roughly 1/2 the world is ocean)
				set_cell(0, Vector2i(pos.x - (width/2) + x, pos.y - (height/2) + y), 1, Vector2(5, 1)) # Change x value where I've wrote three to whatever the x-coord of your oceans are
				...
				set_cell(0, Vector2i(pos.x - (width/2) + x, pos.y - (height/2) + y), 1, Vector2(5,1))
